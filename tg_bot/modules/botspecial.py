...
def insults(bot: Bot, update: Update):
    ...
    text = random.choice((chat.id, "INSULTS-K"))
...
def get_id(bot: Bot, update: Update, args: List[str]):
    ...
    if user_id:
        if update.effective_message.reply_to_message and update.effective_message.reply_to_message.forward_from:
            ...
            update.effective_message.reply_text((chat.id, "{}'s id is `{}`.").format(escape_markdown(user.first_name), user.id),
                                                parse_mode=ParseMode.MARKDOWN)
    else:
        chat = update.effective_chat  # type: Optional[Chat]
        if chat.type == "private":
            update.effective_message.reply_text((chat.id, "Your id is `{}`.").format(chat.id),
                                                parse_mode=ParseMode.MARKDOWN)

        else:
            update.effective_message.reply_text((chat.id, "This group's id is `{}`.").format(chat.id),
                                                parse_mode=ParseMode.MARKDOWN)
...
def markdown_help(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text((chat.id, "Try forwarding the following message to me, and you'll see!"))
    update.effective_message.reply_text((chat.id, "/save test This is a markdown test. _italics_, *bold*, `code`, "
